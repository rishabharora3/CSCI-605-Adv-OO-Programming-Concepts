...
//        imageVectors = new Vector[]{addImageCharToVectorObject(file[0]), addImageCharToVectorObject(file[1])};
        createIOConnections(args[7], Integer.parseInt(args[9]));
    ...
    public static void createIOConnections(String host, int port) {
        try {
            ...
            /*outputStream = new ObjectOutputStream(new BufferedOutputStream(aSocket.getOutputStream()));
//            inPutStream = new BufferedReader(new InputStreamReader(aSocket.getInputStream()));
            inputStream = new ObjectInputStream(new BufferedInputStream(aSocket.getInputStream()));
   */
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
    ...
    public static void sendDataToServer(String data) {
        try {
            ...
            inputStream = new ObjectInputStream(dis);
            if (data.contains("superman.txt")) {
                imageVectors[1] = (Vector<String>) inputStream.readObject();
            } else if (data.contains("batman.txt")) {
                imageVectors[0] = (Vector<String>) inputStream.readObject();
            }
    ...
    private static void sendToServer(Object data) throws IOException {
    ...
    private static void sendPacket(DataOutputStream dos, byte[] bytes) throws IOException {
        ...
        dos.write(bytes);        //Invalid input stream
        dos.flush();
    }
